    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (line.startsWith('package ')) {
        ...
        if (packageMatch) {
          ...
          chunks.push(
            this.createChunk('package', packageName, line, i + 1, i + 1, { exports: [packageName] })
          );
        }
        break;
      }
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (trimmed === 'import (') {
        ...
        if (singleImportMatch) {
          ...
          chunks.push(
            this.createChunk('import', importName, line, i + 1, i + 1, {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (trimmed.startsWith('type ') && trimmed.includes('struct')) {
        ...
        if (structMatch) {
          ...
          const structEnd = this.findStructEnd(lines, i);

          const structContent = lines.slice(i, structEnd).join('\n');

          chunks.push(
            this.createChunk('struct', structName, structContent, structStart, structEnd, {
              exports: [structName],
      ...
      // Parse interface definitions
        ...
        if (interfaceMatch) {
          ...
          const interfaceEnd = this.findInterfaceEnd(lines, i);
      ...
      // Parse type aliases
        ...
        if (typeMatch) {
          ...
          chunks.push(
            this.createChunk('type', typeName, line, i + 1, i + 1, { exports: [typeName] })
          );
        }
      }
    }
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (trimmed.startsWith('func ')) {
        ...
        if (methodMatch) {
          ...
          chunks.push(
            this.createChunk('method', methodName, funcContent, funcStart, funcEnd, {
              parent: receiverType,
        ...
        // Parse regular function
          ...
          if (funcMatch) {
            ...
            chunks.push(
              this.createChunk('function', funcName, funcContent, funcStart, funcEnd, {
                parameters: this.extractParameters(trimmed),
                exports: [funcName],
              })
            );
  ...
  private parseConstants(lines: string[]): SemanticChunk[] {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (trimmed === 'const (') {
        ...
        chunks.push(
          this.createChunk('constant', 'constants', constBlock.join('\n'), blockStart, i + 1, {
        ...
        if (constMatch) {
          ...
          chunks.push(
            this.createChunk('constant', constName, line, i + 1, i + 1, { exports: [constName] })
          );
        }
      }
    }
  ...
  private parseVariables(lines: string[]): SemanticChunk[] {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (trimmed === 'var (') {
        ...
        chunks.push(
          this.createChunk('variable', 'variables', varBlock.join('\n'), blockStart, i + 1, {
        ...
        if (varMatch) {
          ...
          chunks.push(
            this.createChunk('variable', varName, line, i + 1, i + 1, { exports: [varName] })
          );
        }
      }
    }
