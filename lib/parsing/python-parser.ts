...
export class PythonParser extends BaseParser {
  ...
  private parseImports(lines: string[]): SemanticChunk[] {
    ...
    if (currentImportBlock.length > 0) {
      chunks.push(
        this.createChunk('import', 'imports', currentImportBlock.join('\n'), blockStart, blockEnd, {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (trimmed.startsWith('class ')) {
        ...
        if (classMatch) {
          ...
          chunks.push(
            this.createChunk('class', className, classContent, classStart, classEnd, {
              exports: [className],
    ...
    for (let i = classStart + 1; i < classEnd; i++) {
      ...
      if (trimmed.startsWith('def ') && indentation > 0) {
        ...
        if (methodMatch) {
          ...
          const methodEnd = this.findMethodEnd(lines, i, indentation);
          ...
          chunks.push(
            this.createChunk('method', methodName, methodContent, methodStart, methodEnd, {
              parent: className,
              visibility,
              async: isAsync,
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (
        ...
        if (assignmentMatch) {
          ...
          chunks.push(
            this.createChunk('constant', varName, line, i + 1, i + 1, { exports: [varName] })
          );
        } else {
  ...
  private parseDecorators(lines: string[]): SemanticChunk[] {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (
        trimmed.startsWith('@') &&
        !trimmed.includes('staticmethod') &&
        !trimmed.includes('classmethod') &&
        ...
        if (decoratorMatch) {
          ...
          chunks.push(
            this.createChunk('function', `decorator-${decoratorName}`, line, i + 1, i + 1, {
