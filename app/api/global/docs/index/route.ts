...
const DOC_PATTERNS = [
  ...
  'docs/**/*.md', 'docs/**/*.rst', 'documentation/**/*.md',
  // API documentation
  'api.md', 'API.md', 'api/**/*.md',
  // Getting started guides
  'getting-started.md', 'quickstart.md', 'tutorial.md',
  // Examples
  'examples/**/*.md', 'example/**/*.md',
  // Changelog and migration
  'CHANGELOG.md', 'changelog.md', 'MIGRATION.md',
  // Contributing guides
...
const CODE_PATTERNS = [
  'examples/**/*.js', 'examples/**/*.ts', 'examples/**/*.jsx', 'examples/**/*.tsx',
  'example/**/*.js', 'example/**/*.ts', 'example/**/*.jsx', 'example/**/*.tsx',
  'demo/**/*.js', 'demo/**/*.ts', 'demo/**/*.jsx', 'demo/**/*.tsx',
...
function detectFileType(filePath: string): { language: string; type: 'documentation' | 'code' } {
  ...
  if (ext === '.txt' && basename.includes('readme')) return { language: 'text', type: 'documentation' };
...
async function indexDocumentationSource(repo: DocRepository): Promise<{ success: boolean; documentCount: number; error?: string }> {
...
async function indexGitRepository(repo: DocRepository): Promise<{ success: boolean; documentCount: number; error?: string }> {
...
// Index llms.txt file
async function indexLLMsTxtFile(repo: DocRepository): Promise<{ success: boolean; documentCount: number; error?: string }> {
  try {
...
async function indexWebsite(repo: DocRepository): Promise<{ success: boolean; documentCount: number; error?: string }> {
  try {
    ...
    for (let i = 0; i < pagesToProcess.length; i += batchSize) {
      ...
      const batchPromises = batch.map(async (url, batchIndex) => {
        try {
          ...
          if (result.chunks.length > 0) {
            ...
            if (!result.success) {
              console.log(`Using fallback parsing for ${sourceName} - got ${result.chunks.length} chunks`);
            }
          } else {
          ...
          console.log(`Indexed ${pageDocumentCount} chunks from ${url}`);
      ...
      console.log(`Completed batch ${Math.floor(i / batchSize) + 1}, indexed ${batchTotal} chunks in this batch`);
    ...
    console.log(`Successfully indexed website with ${totalDocumentCount} total content chunks from ${pagesToProcess.length} pages`);
...
function extractContentFromHTML(html: string): string {
  try {
    ...
    const contentStrategies = [
      ...
      () => {
        const headingsContent = content.match(/<h[1-6][^>]*>([\s\S]*?)<\/h[1-6]>[\s\S]*?(?=<h[1-6]|$)/gi);
    ...
    for (const strategy of contentStrategies) {
      ...
      if (result && result.trim().length > 100) {
        ...
        console.log(`Successfully extracted content using strategy ${contentStrategies.indexOf(strategy) + 1}`);
        break;
      }
    }
...
async function discoverWebsitePages(baseUrl: string): Promise<string[]> {
  ...
  try {
    ...
    console.log(`Found ${Math.min(sitemapUrlsFromOtherPaths.length, maxOtherUrls)} URLs from other paths (limited from ${sitemapUrlsFromOtherPaths.length})`);
    ...
    if (sitemapUrlsUnderBasePath.length < 20) {
      console.log(`Only found ${sitemapUrlsUnderBasePath.length} sitemap URLs under base path, doing comprehensive link crawling...`);
      const crawledUrls = await comprehensiveLinkCrawling(pagesToCrawlForLinks, baseDomain, baseUrlPath);
      ...
      .filter(url => {
        try {
          ...
          const excludePatterns = [
            '/api/', '/login', '/signup', '/auth', '/contact', '/about',
            '.pdf', '.zip', '.tar', '.gz', '.json', '.xml', '.rss',
            '/images/', '/css/', '/js/', '/fonts/', '/static/'
          ];
    ...
    console.log(`Final URL list: ${urlsUnderBasePath.length} pages under base path "${baseUrlPath}" (unlimited), ${urlsFromOtherPaths.length} pages from other paths (limited)`);
  ...
  for (const sitemapUrl of sitemapUrls) {
    try {
      ...
      if (sitemapUrl.endsWith('.xml')) {
        ...
        const sitemapRefs = content.split('\n')
          .map(line => line.trim())
...
async function comprehensiveLinkCrawling(seedUrls: string[], baseDomain: string, baseUrlPath: string): Promise<string[]> {
  const discoveredLinks = new Set<string>();
  const processedUrls = new Set<string>();
  const toProcess = [...seedUrls];
  
  console.log(`Starting comprehensive link crawling with ${seedUrls.length} seed URLs under path "${baseUrlPath}"`);
  ...
  while (toProcess.length > 0 && wave < maxWaves) {
    ...
    console.log(`Wave ${wave} completed: found ${newLinksFound} new links, ${toProcess.length} URLs queued for next wave`);
  ...
  console.log(`Comprehensive link crawling completed after ${wave} waves: discovered ${finalLinks.length} total links`);
...
function createManualChunks(content: string): string[] {
  ...
  for (const paragraph of paragraphs) {
    ...
    if (currentChunk.length + cleanParagraph.length > maxChunkSize && currentChunk.length > minChunkSize) {
